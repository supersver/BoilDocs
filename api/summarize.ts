...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!webhookUrl) {
    ...
    return res
      .status(500)
      .json({ error: "Server misconfiguration: webhook URL is missing" });
  }
  ...
  if (!webhookSecret) {
    ...
    return res
      .status(500)
      .json({ error: "Server misconfiguration: webhook secret is missing" });
  }
  ...
  try {
    ...
    return res
      .status(502)
      .json({ error: "Could not reach the summarization service" });
  }
  ...
  try {
    ...
    console.error(
      "[/api/summarize] n8n returned an invalid JSON response:",
      err,
    );
    return res
      .status(502)
      .json({ error: "Summarization service returned an invalid response" });
  }
}
